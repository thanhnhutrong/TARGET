...
class Server:
    def __init__(self):
        ...
        self.BRIDGE_HOST = os.environ.get("BRIDGE_HOST", "70.31.197.180")
        self.BRIDGE_PORT = int(os.environ.get("BRIDGE_PORT", 41052))
        self.sim = 0
        self.ego = 0
    ...
    def process_with_data_lgsvl_part(self, data_to_process:dict):
        ...
        try:
            ...
            if map_of_lgsvl == 'borregas_ave':
                if self.sim.current_scene == 'aae03d2a-b7ca-4a88-9e41-9035287a12cc':
                    self.sim.reset()
                else:
                    self.sim.load('aae03d2a-b7ca-4a88-9e41-9035287a12cc')
                ...
                if self.sim.current_scene == '12da60a7-2fc9-474d-a62a-5cc08cb97fe8':
                    ...
                    self.sim.load('12da60a7-2fc9-474d-a62a-5cc08cb97fe8')
            ...
            ego_state.velocity = 15 * forward

            if self.ADS_model == 'apollo5':
                if data_to_process["ego"]["groundTruthPerception"]:
                    self.ego = self.sim.add_agent('8e776f67-63d6-4fa3-8587-ad00a0b41034', lgsvl.AgentType.EGO, ego_state)
                else:
                    self.ego = self.sim.add_agent('8e776f67-63d6-4fa3-8587-ad00a0b41034', lgsvl.AgentType.EGO, ego_state)
            ...
            self.ego.connect_bridge(os.environ.get("BRIDGE_HOST", "70.31.197.180") ,port = 41052)
    ...
    def process_with_data_apollo_part(self, data_to_process:dict):
        ...
        dv = lgsvl.dreamview.Connection(self.sim, self.ego, self.BRIDGE_HOST, "41352")
        # if map_of_lgsvl== 'borregas_ave':
        ...
        dv.set_destination(destination_x, destination_y, coord_type=coord_type)
    ...
    def get_port(self):
        return os.getenv('WS_PORT', '8999')

    def get_host(self):
        return os.getenv('WS_HOST', '0.0.0.0')

    def start(self):
        return websockets.serve(self.handler, '0.0.0.0', 8999 ,ping_interval=None)
    ...
    async def handler(self, websocket, path):
      async for message in websocket:
        ...
        # if msg['CMD'] != 'KEEP_SERVER_AND_CLIENT_ALIVE':
        #     print("Message: ", msg)
